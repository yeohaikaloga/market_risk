...
def generate_product_generic_curves(product: str, cob_date: str, window: int) -> Dict[str, Any]:
    ...
    product_instrument_map = {
        'cotton': ['CT', 'VV', 'CCL', 'S ', 'W ', 'C ', 'SB'],
        'rubber': ['BDR', 'OR', 'JN', 'RG', 'RT', 'SRB'],
        'fallback': ['CT']
    }
    instruments_list = product_instrument_map.get(product, ['CT'])
...
def prepare_pos_data_for_var(combined_pos_df: pd.DataFrame, method: str) -> pd.DataFrame:
    ...
    base_cols = [
        'cob_date', 'product', 'unit', 'region', 'position_type', 'product_code',
