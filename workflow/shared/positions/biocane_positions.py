...
# TODO biocane positions code

def generate_biocane_combined_position(cob_date: str, instrument_dict: Dict[str, Any], prices_df: pd.DataFrame,
    ...
    outright_phy_pos_df = physical_loader.load_biocane_phy_position_from_staging(cob_date=cob_date)
    outright_phy_pos_df['instrument_name'] = outright_phy_pos_df['product'].apply(
        lambda x: 'GARMMZ SUGAR' if x == 'Sugar' else ('MAIZE UP' if x == 'Maize-E' else None)
    )
    ...
    outright_phy_pos_df['to_USD_conversion'] = 10
    ...
    outright_phy_pos_df['position_index'] = (
            product[:3] + '_L_' + str(cob_date) + '_' +
            outright_phy_pos_df.index.map(lambda i: str(i).zfill(4))
    )
