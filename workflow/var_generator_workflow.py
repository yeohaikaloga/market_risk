...
def generate_var_workflow(product, method, cob_date, window):
    if product == 'cotton':
        return generate_cotton_var_workflow(method, cob_date, window)
    elif product == 'rubber':
        pass
        #TODO write generate_rubber_var_workflow
        #return generate_rubber_var_workflow(method, cob_date, window)
    elif product == 'rms':
        pass
        # TODO write generate_rms_var_workflow
        #return generate_rms_var_workflow(method, cob_date, window)
    elif product == 'wood':
        pass
        # TODO write generate_wood_var_workflow
        #return generate_wood_var_workflow(method, cob_date, window)
    elif product == 'biocane':
        pass
        # TODO write generate_biocane_var_workflow
        # return generate_biocane_var_workflow(method, cob_date, window)
    else:
...
def generate_cotton_var_workflow(method, cob_date, window) -> dict[str, pd.DataFrame]:
    """
        Generates cotton VaR data for a given method.
    ...
    uat_engine = get_engine('uat')
     # NOTE 'for 4 Aug COB, need to put in 2025-08-05'
    ...
    for instrument_name in instruments_list:
        ...
        generic_curves_df, active_contracts_df = price_series.generate_generic_curves_df_up_to(max_position=max_position,
                                                                          roll_days=14,
                                                                          adjustment='ratio',
    ...
    deriv_pos_df['region'] = deriv_pos_df['unit'].map(cotton_unit_region_mapping) # Overwrite unit-region in DB with that from CONSOLIDATE tab
    ...
    combined_pos_df = combined_pos_df[['cob_date', 'product', 'position_index', 'unit', 'region', 'books',
                                       'position_type', 'exposure', 'trader_id', 'trader_name', 'counterparty_id',
                                       'counterparty_parent', 'instrument_name', 'bbg_ticker', 'underlying_bbg_ticker',
                                       'generic_curve', 'delta', 'to_USD_conversion']]
    print(combined_pos_df.head())
    validate_combined_position_df(combined_pos_df)
    #check_pivot_positions(combined_pos_df, cob_date, units='delta')
    ...
    filename = f"{cob_date}_pnl_output_{timestamp}.xlsx"
    with pd.ExcelWriter(filename) as writer:
        combined_pos_df.to_excel(writer, sheet_name='combined_position')
        ...
        instrument_dict['CT']['relative_returns_$_df'].to_excel(writer, sheet_name='ct_rel_ret')

    return {'delta': combined_pos_df,
            'main': {'var': var_table_df, 'outright_lookback_pnl': outright_lookback_pnl_df,
                     'outright_inverse_pnl': outright_inverse_pnl_df, 'basis_lookback_pnl': basis_lookback_pnl_df},
            'price': {'var': price_var_table_df},
            'instrument': {'ref': instrument_ref_dict, 'prices': instrument_dict}}
